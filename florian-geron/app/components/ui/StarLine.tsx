...
export function StarLine({
  ...
  const clamped = isNa ? null : Math.max(0, Math.min(rating, max));
  const percent = clamped == null ? null : (clamped / max) * 100;
  const ratioText = clamped == null ? 'n/a' : `${clamped}/${max}`;

  return (
    <div className={`w-full ${className}`} title={ratioText} aria-label={isNa ? 'Rating not available' : `Rating ${clamped} out of ${max}`} role="img">
      <div className="relative h-16 pr-16">
        ...
        <div className="absolute left-0 right-0 top-0 flex items-center justify-between text-sm text-gray-600">
          ...
          <div className="text-right">{ratioText}</div>
        </div>
